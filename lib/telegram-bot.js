...
export class ApplicationBot {
    constructor(token, groupChatId) {
        this.activeThreads = new Map(); // Хранит thread_id по идентификатору пользователя
        if (!token) {
    ...
    async handleNewApplication(application) {
        try {
            ...
            if (!threadId) {
                ...
                const topic = await this.bot.api.createForumTopic(this.groupChatId, topicName);
                ...
                await this.bot.api.sendMessage(this.groupChatId, message, { message_thread_id: threadId });
            }
            else {
